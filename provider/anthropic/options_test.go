...
const okBody = `{"id":"m","model":"x","role":"assistant","content":[{"type":"text","text":"ok"}],` +
...
func TestClientRetriesOn503(t *testing.T) {
	...
	resp, err := c.Complete(context.Background(),
		golm.Request{Model: "claude", Messages: []golm.Message{golm.UserText("hi")}, MaxTokens: 16})
	if err != nil {
...
func TestWithHTTPClientIsTheOneUsed(t *testing.T) {
	...
	if _, err := c.Complete(context.Background(),
		golm.Request{Model: "claude", Messages: []golm.Message{golm.UserText("hi")}, MaxTokens: 16}); err != nil {
		t.Fatalf("Complete: %v", err)
	}
